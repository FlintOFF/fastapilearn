...
class BookAuthor(Base):
    ...
    def __repr__(self):
        return "<BookAuthor (bookauthor_id='{0}', author_id='{1}', book_id='{2}', first_name='{3}', last_name='{4}', title='{5}')>" \
            .format(self.bookauthor_id, self.author_id, self.book_id, self.author.first_name, self.author.last_name,
                    self.book.title)
...
def add_book(book: Book, author: Author):
    with Session(engine) as session:
        ...
        if existing_author is not None:
            ...
            print("Author does not exist. Adding author and book and pair")
            session.add(author)
            session.flush()
